...
public interface Basic {
	 ...
	 *  -인터페이스는 다중 상속을 허용한다.
	 *  (결국 메서드에 대한 정의만 하고 있어서 메서드가 겹치더라도 최종 구현 부분은 구현 클래스에서 이루어 질 것이기 때문에 다중상속(구현)이 가능)
	 ...
	 * 
	 * 2.차이점
	 * -추상클래스는 클래스 내에 인스턴스 변수, 메서드를 생성이 가능하고, 추상메서드가 포함되었거나
	 * abstract키워드로 클래스가 정의되어 있고
	 * 인터페이스는 인스턴스 변수, 메서드 생성이 불가능하며 모든 변수는 상수필드, 모든 메서드는 추상메서드로 정의되어있다.
	 * 
	 * -존재하는 목적이 다르다.
	 * 추상클래스는 추상클래스를 상속받아서 기능을 이용하고, 클래스를 확장하는데 목적이있다.
