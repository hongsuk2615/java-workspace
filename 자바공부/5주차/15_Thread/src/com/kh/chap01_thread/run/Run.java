...
public class Run {
	 ...
	 * 
	 * 스레드 - 한개의 프로세스(process)내에서 실제로 작업을 수행하는 소단위 - 모든 프로세스에는 한개 이상의 스레드가 존재하여 작ㅈ업을
	 * 수행함
	 * 
	 * 멀티스레드 - 한개의 프로그램을 실행하고 그 내부적으로 여러 작업(스레드)를 처리하는 것 - 멀티스레드의 장점 1) 자원을 효율적으로 사용
	 * 할 수 있다. 2) 사용자 입장에서 일처리가 빠르게 보인다. 3) 작업이 분리되어 코드가간결해 진다.
	 * 
	 * ex) - 청소(나, 1시간) , 빨래(세탁기, 2시간) , 설거지(나, 30분) , 화장실 청소(나, 30분) - 무조건 주체가 되는
	 * 중심이 되는 스레드가 메인스레드 == 자바의 메인메서드 - 추가로 필요한 쓰레드 == 서브스레드 -> main메서드에서 생성해서 실행
	 * 한다. - (서브스레드 예시) : 비행기 게임, 좀보이드
	 * 
	 */
