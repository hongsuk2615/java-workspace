...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		//1. 별도의 경로지정을 하진 않고 파일명(test.txt)만 제시해서 생성해보기 => 경로는 현재 디렉토리
		try {
			File file1 = new File("test.txt");
			file1.createNewFile();
			//2. 경로지정을 한 파일명 (C:\\aaa\\test.txt)을 제시해서 생성하기 => 이 때 존재하는 경로를 제시해야함
			...
			//4. 별도의 경로 지정 없이 폴더 생성 후 파일 생성하기 => 프로젝트 폴더 내에 생성 될 것.
		...
		/*
		 * 프로그램 상의 데이터를 외부매체(모니터, 스피커, 자바프로그램, 파일 등등)로 출력하거나 입력장치(키보드,마우스등)으로 
		 ...
		 * 스트림 구분
		 * -통로의 사이즈 :
		 * 	      바이트 스트림 : 1byte짜리가 이동할 수 있을정도의 사이즈(좁은 통로) => 입력(XXXInputStream)/출력(XXXOutputStream)
		 ...
		 *                  단, 보조스트림은 단독적으로 사용이 불가능하므로 반드시 외부매체와 직접적으로 연결가능한 기반 스트림과 함께 사용해야한다.
		 * 
		 */
