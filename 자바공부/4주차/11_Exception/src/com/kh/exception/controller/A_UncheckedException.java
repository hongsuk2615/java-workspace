...
public class A_UncheckedException {
	 ...
	 * 레퍼런스가 아직 null임에도 불구하고 접근하려고 할때 발생하는 예외 -ArithmeticException : 나누기 연산시 0으로 나눌때
	 * 발생하는 예외 -......
	 * 
...
//		System.out.println("프로그램 종료");
		...
		/*
		 * try ~ catch문 [표현법] try{ //예외가 발생될수 있는 구문 }catch (발생될 예외클래스 변수명 -> 매개변수){ //예외
		 * 발생시 실행할 구문 }
		 */
...
//		if(size>0) { //방법1. if문으로 size값이 양수값일 때만 생성하도록 제어.
...
//					+ "인덱스로 접근한 것 같습니다.");
//		} //다중 catch블럭이 줄어들었으나 정확히 어떤 에러인지 알 수 없음
		...
		} // 다중 catch에서는 블럭의 순서가 중요함. => 범위가 작은 자식타입의 예외 클래스를 먼저 기술 해야함.
	}
	/*
	 * RuntimeException 관련된 예외는
	 * -조건문으로 해결가능 => 예외 자체가 발생 안되게끔 개발자가 소스코드로 핸들링 가능
